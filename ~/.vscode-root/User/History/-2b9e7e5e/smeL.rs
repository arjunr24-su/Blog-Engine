...
async fn posts_by_category(tag: String, page: Option<u32>) -> Template {
    ...
    let pagination = Pagination {
        current_page: page,
        has_previous: page > 1,
        has_next: page * articles_per_page < filtered_articles.len(),
        previous_page_url: if page > 1 { Some(format!("/category/{}?page={}", tag, page - 1)) } else { None },
        next_page_url: if page * articles_per_page < filtered_articles.len() { Some(format!("/category/{}?page={}", tag, page + 1)) } else { None },
        pages: (1..=((filtered_articles.len() as f32 / articles_per_page as f32).ceil() as u32)).map(|p| Page { number: p, url: format!("/category/{}?page={}", tag, p) }).collect(),
    };
